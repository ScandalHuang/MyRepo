...
namespace test.Controllers
{
    public class WeixinController : ApiController
    {
        public HttpResponse Get([FromUri]UrldecodeModel model)
        {
            ...
            //var sVerifyMsgSig = HttpUtility.UrlDecode(HttpContext.Current.Request["msg_signature"] ?? string.Empty);
            //var sVerifyTimeStamp = HttpUtility.UrlDecode(HttpContext.Current.Request["timestamp"] ?? string.Empty);
            //var sVerifyNonce = HttpUtility.UrlDecode(HttpContext.Current.Request["nonce"] ?? string.Empty);
            //var sVerifyEchoStr = HttpUtility.UrlDecode(HttpContext.Current.Request["echostr"] ?? string.Empty);
            int ret = 0;
            ...
            //return Content(HttpStatusCode.OK, sEchoStr);
        }
        public HttpResponse Post([FromUri]UrldecodeModel model)
        {
            ...
            string sVerifyTimeStamp = HttpUtility.UrlDecode(model.timestamp);
            string sVerifyNonce = HttpUtility.UrlDecode(model.nonce);
            //string msg_signature = HttpUtility.UrlDecode(HttpContext.Current.Request["msg_signature"] ?? string.Empty);
            //string nonce = HttpUtility.UrlDecode(HttpContext.Current.Request["nonce"] ?? string.Empty);
            //string timestamp = HttpUtility.UrlDecode(HttpContext.Current.Request["timestamp"] ?? string.Empty);
            ...
            //var postData = Encoding.UTF8.GetString(b);
