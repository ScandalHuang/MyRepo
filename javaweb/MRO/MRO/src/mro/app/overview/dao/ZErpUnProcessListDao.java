...
public class ZErpUnProcessListDao extends FactoryBaseDAO {

	public ZErpUnProcessListDao(SessionFactory sessionFactory) {
	...
	public List getList(String condition, Map param) {
		...
		String sql = "SELECT  (select lm.LOCATION_SITE from location_map lm "
				...
				+ "PART_NO_DESC \"品名敘述\", (SELECT MIN_BASIC_UNIT "
				+ "FROM ITEM_ATTRIBUTE WHERE ITEMID=(SELECT ITEMID FROM ITEM "
				+ "WHERE ITEMNUM=B.part_no)) \"單位\", "
				...
				+ "DISPATCH_DATE \"分發採購員日期\",LINE_MEMO \"其他說明\",NOTE_TO_AGENT \"採購員說明\" "
				+ "FROM Z_ERP_UNPROCESS_PR b "
				+ "where 1=1 " + condition;
