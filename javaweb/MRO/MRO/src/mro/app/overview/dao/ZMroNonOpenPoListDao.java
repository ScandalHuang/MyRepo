...
public class ZMroNonOpenPoListDao extends FactoryBaseDAO {

	public ZMroNonOpenPoListDao(SessionFactory sessionFactory) {
	...
	public List getList(String condition, Map param) {
		...
		String sql = "SELECT   (select lm.LOCATION_SITE from location_map lm "
				...
				+ "FROM item i LEFT JOIN  CLASSSTRUCTURE c  ON i.CLASSSTRUCTUREid = c.CLASSSTRUCTUREid "
				...
				+ "B.PART_NO_DESC \"品名敘述\", "
				+ "(SELECT MIN_BASIC_UNIT FROM ITEM_ATTRIBUTE WHERE ITEMID=(SELECT ITEMID FROM ITEM WHERE ITEMNUM=B.part_no)) \"單位\","
				+ "PR_NO \"請購單編號\",PR_LINE_NO \"請購單項次\", "
				+ "PO_NO \"訂單編號\",PO_LINE_NO \"訂單項次\",QUANTITY \"訂單量\", "
				+ "(b.QUANTITY - b.RECEIVED_QTY) \"未交量\",UNDELIVER_QTY \"IQC\","
				+ "RECEIVED_QTY \"收料量\", BILL_QTY \"立帳量\","
				+ "(RECEIVED_QTY - BILL_QTY) \"已收未立帳數量\", "
				...
				+ "WHERE person_id = SUBSTR (b.BUYER_EMPLOYEE_NUMBER, -8, 8)) \"採購員\", "
				+ "REQUESTOR_EMP_NO \"請購人工號\",REQUESTOR_NAME \"請購人姓名\",REQUESTOR_DEPT_CODE \"請購人部門\", "
				+ "INSPECTOR_EMP_NUM \"驗收人工號\",INSPECTOR_EMP_NAME \"驗收人姓名\", "
				+ "INSPECTOR_DEPT_CODE \"驗收人部門\",DISPATCH_DATE \"分發採購員日期\",LINE_MEMO \"其他說明\", "
				+ "NOTE_TO_AGENT \"採購員說明\" FROM   Z_ZPO_MRO_NON_OPEN_PO B "
				+ "where 1=1 " + condition;
