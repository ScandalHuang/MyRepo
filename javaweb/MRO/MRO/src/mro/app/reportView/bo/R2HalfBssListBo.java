...
import mro.app.reportView.dao.R2HalfBssListDao;
...
@Component
@Scope("prototype")
public class R2HalfBssListBo {

    private R2HalfBssListDao dao;
    
    @Autowired
    @Qualifier(value="DEFAULT_SESSION_FACTORY")
    public void initDAO(SessionFactory sessionFactory) {
    	dao=new R2HalfBssListDao(sessionFactory);
    }
    ...
    public List getList(String locationSite,String[] organizationCode,String deptNo,
		...
		if(StringUtils.isNotBlank(locationSite)){
			condition.append("AND exists (select 1 from location_map lm "
					+ "where a.SITEID=lm.site_id and lm.LOCATION_SITE=:locationSite)");
